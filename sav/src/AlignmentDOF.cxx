...
void LegendreDOFSet::fillDerivatives(const DerivativeContext& ctx, Eigen::Ref<Eigen::MatrixXd> out) const
{
  ...
  const auto [u, v] = o2::alignment::computeUV(gloX, gloY, ctx.measZ, ctx.sensorID, o2::its3::constants::radii[ctx.layerID]);
  const auto pu = o2::alignment::legendrePols(mOrder, u);
  const auto pv = o2::alignment::legendrePols(mOrder, v);
  const double phiWidth = o2::alignment::getSensorPhiWidth(ctx.sensorID, o2::its3::constants::radii[ctx.layerID]);
  ...
  if (std::abs(u) > o2::constants::math::Almost0) {
    ...
    for (int iq = 0; iq < 8; ++iq) {
      ...
      const auto puQ = o2::alignment::legendrePols(mOrder, up);
      int idx = 0;
